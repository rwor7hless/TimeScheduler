...
function SortableRow({ id, children }: { id: number; children: React.ReactNode }) {
  ...
  //
  // ВНИМАНИЕ: этот обход появился, когда у карточек были тень и
  // backdrop-filter — они создавали содержащий блок и лишние слои отрисовки,
  // и во время slide-анимации соседние тени просвечивали друг сквозь друга
  // как «блик». Ни теней, ни блюра в терминальном редизайне не осталось,
  // так что обход, скорее всего, больше не нужен. Спек запрещает снимать его
  // вслепую: удалите три свойства ниже, перетащите строку и оставьте только
  // то, что реально понадобилось.
          ...
          <div className="space-y-1.5">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="primary-btn px-3 py-2 transition-colors"
              >
            ...
            return (
              <section key={keyStr} className="pt-1">
                <button
                  ...
                  className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-fg-mid hover:text-fg transition-colors"
                >
                  <svg
