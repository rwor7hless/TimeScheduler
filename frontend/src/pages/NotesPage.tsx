...
function TaskPicker({ tasks, value, onChange }: TaskPickerProps) {
  ...
  const selected = value ? tasks.find(t => t.id === value) : null
  ...
  const select = (taskId: number | null) => {
    onChange(taskId)
    setOpen(false)
    setSearch('')
  }
    ...
    <div ref={containerRef} className="relative flex-1 min-w-0">
      <button
        ...
        className={[
          ...
          selected ? 'text-amber-700 dark:text-amber-400' : 'text-gray-500 dark:text-gray-400',
        ].join(' ')}
      >
        {selected ? (
          <>
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={['ml-auto shrink-0 transition-transform', open ? 'rotate-180' : ''].join(' ')}>
        ...
        <div className="absolute left-0 top-full mt-1 z-50 w-72 max-w-[calc(100vw-2rem)] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg overflow-hidden">
          <div className="p-2 border-b border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
              ...
              <input ref={searchRef} type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск задачи..." className="flex-1 text-xs bg-transparent border-none outline-none text-gray-800 dark:text-gray-200 placeholder-gray-400" />
              {search && (
          ...
          <div className="max-h-52 overflow-y-auto py-1">
            <button type="button" onClick={() => select(null)} className={['flex items-center gap-2.5 w-full px-3 py-2 text-xs transition-colors touch-manipulation text-left', !value ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50'].join(' ')}>
              <span className="w-4 h-4 flex items-center justify-center shrink-0">
                {!value && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}
              </span>
              Без задачи
            </button>
            {filtered.length === 0 ? (
              <p className="px-3 py-3 text-xs text-gray-400 dark:text-gray-500 text-center">Задачи не найдены</p>
            ) : (
              filtered.map(t => (
                <button key={t.id} type="button" onClick={() => select(t.id)} className={['flex items-center gap-2.5 w-full px-3 py-2 text-xs transition-colors touch-manipulation text-left', t.id === value ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50'].join(' ')}>
                  <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: t.color || '#D97706' }} />
                  <span className="truncate">{t.title}</span>
                  {t.id === value && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="ml-auto shrink-0"><polyline points="20 6 9 17 4 12"/></svg>}
                </button>
              ))
            )}
          </div>
...
function NoteCard({ note, isSelected, task, onClick }: NoteCardProps) {
  ...
  return (
    <button type="button" onClick={onClick} className={['w-full text-left px-3 py-3 rounded-lg transition-colors touch-manipulation', isSelected ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800' : 'hover:bg-gray-50 dark:hover:bg-gray-800 border border-transparent'].join(' ')}>
      <div className="flex items-start justify-between gap-2">
        <span className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{note.title || 'Без названия'}</span>
        <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0 mt-0.5">{formatDate(note.updated_at)}</span>
      </div>
      {task && <span className="inline-block mt-1 text-xs px-1.5 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800 truncate max-w-full">{task.title}</span>}
      {preview && <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed">{preview}</p>}
    </button>
  )
}
...
export default function NotesPage() {
  ...
  useEffect(() => {
    if (selectedNote) {
      ...
      setDraftTaskId(selectedNote.task_id)
    }
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      updateNote.mutate({
        id: selectedId,
        data: { title: draftTitle, content: draftContent, task_id: draftTaskId },
      })
    }, 600)
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current) }
  }, [draftTitle, draftContent, draftTaskId]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleCreate = useCallback(async () => {
    try {
      const note = await createNote.mutateAsync({ title: '', content: '', task_id: null })
      selectNote(note.id)
    } catch {
  ...
  const handleFormat = useCallback((type: FormatType) => {
    ...
    applyFormat(textareaRef.current, type, setDraftContent)
  }, [])
  ...
  const taskForNote = (note: NoteResponse) => note.task_id ? tasks.find(t => t.id === note.task_id) : undefined

  // ── Sidebar ──────────────────────────────────────────────────────────────
  const noteList = (
    <div className={['flex flex-col border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900', 'lg:w-64 lg:flex-shrink-0', mobileView === 'list' ? 'flex flex-col flex-1' : 'hidden lg:flex'].join(' ')}>
      <div className="px-3 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between gap-2">
        ...
        <button type="button" onClick={handleCreate} title="Новая заметка" disabled={createNote.isPending} className="flex items-center justify-center w-8 h-8 rounded-lg bg-amber-500 hover:bg-amber-600 text-white transition-colors touch-manipulation disabled:opacity-50">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-16 text-center px-4">
            ...
            <button type="button" onClick={handleCreate} className="text-sm font-medium text-amber-600 hover:text-amber-700 dark:text-amber-400 dark:hover:text-amber-300">Создать первую</button>
          </div>
        ) : (
          notes.map(note => (
            <NoteCard key={note.id} note={note} isSelected={note.id === selectedId} task={taskForNote(note)} onClick={() => selectNote(note.id)} />
          ))
        )}
      </div>
    </div>
  ...
  // ── Editor ───────────────────────────────────────────────────────────────
  const editor = (
    <div className={['flex flex-col flex-1 min-h-0 bg-white dark:bg-gray-900', mobileView === 'editor' ? 'flex' : 'hidden lg:flex'].join(' ')}>
      {!selectedNote ? (
        ...
        <>
          <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
            <button type="button" onClick={() => setMobileView('list')} className="lg:hidden flex items-center justify-center w-8 h-8 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 touch-manipulation">
            ...
            />
            <button type="button" onClick={() => setEditorMode(m => m === 'edit' ? 'preview' : 'edit')} title={editorMode === 'edit' ? 'Предпросмотр' : 'Редактировать'} className={['flex items-center justify-center w-8 h-8 rounded-lg transition-colors touch-manipulation shrink-0', editorMode === 'preview' ? 'bg-amber-500 text-white' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300'].join(' ')}>
              {editorMode === 'edit' ? (
            ...
            <button type="button" onClick={handleDelete} disabled={deleteNote.isPending} title="Удалить заметку" className="flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors touch-manipulation disabled:opacity-50">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            ...
            <TaskPicker tasks={tasks} value={draftTaskId} onChange={setDraftTaskId} />
          </div>
          {editorMode === 'edit' && <MarkdownToolbar textareaRef={textareaRef} onFormat={handleFormat} />}
          <div className="flex-1 min-h-0 overflow-y-auto">
            {editorMode === 'edit' ? (
              ...
              <div className="px-4 py-3 markdown-preview">
                {draftContent ? (
                  <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]}>{draftContent}</ReactMarkdown>
                ) : (
