...
- **Four CSS guard tests** exist because plan 1's hand-rewrite of `globals.css` broke each of these silently: `css-vars.test.ts` (no dangling `var(--…)`), `css-classes.test.ts` (every project class the markup references is defined), `svg-size.test.ts` (every `<svg>` declares its own size — an unsized one defaults to 300×150px), and `legacy-classes.test.ts` (no Tailwind palette classes, radii, shadows, rings, hex literals, or a second breakpoint)
