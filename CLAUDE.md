...
- **One breakpoint.** `narrow` (`max-width: 899px`), declared once in `tailwind.config.js` under `theme.extend.screens`. Tailwind's default `sm:`/`md:`/`lg:`/`xl:`/`2xl:` prefixes are banned and enforced by `src/styles/legacy-classes.test.ts`; the `container` core plugin is disabled because it dragged those breakpoints back into the bundle. The built stylesheet contains exactly one `@media` query, and a test holds it there
- **Command palette** (`Ctrl+K` / `Cmd+K`, `src/components/palette/`), mounted once in `AppShell`. One input, three behaviours: empty lists commands, typed text merges `/api/search` results under them, free text with no match creates a task via `parseTaskInput`. Bound to `event.code === 'KeyK'`, **not** `event.key`, so it survives a Cyrillic layout (where `event.key` is `'л'`)
- **`MobileTabBar`** (`src/components/layout/MobileTabBar.tsx`) below 900px — four tabs (Мой день / Задачи / Привычки / Статистика) plus a central `+` that opens the palette. There is no burger, drawer or edge-swipe any more; below 900px the sidebar is not rendered at all. Calendar, Notifications and Admin have no tab — they are reached through the palette
- **`MobileTopBar`** (`src/components/layout/MobileTopBar.tsx`), also below 900px only, carries the theme toggle and logout. Both live in the sidebar footer on desktop, and hiding the sidebar made logout unreachable on a phone entirely. The palette carries «Выйти» and «Новый проект» for the same reason. **Renaming and deleting a project is still sidebar-only** — the row context menu has no mobile equivalent
- `CreateProjectModal` (`src/components/boards/`) is shared by the sidebar's board tree and the palette command; it owns its own state and queries so neither caller has to host it
- Navigation lives in `src/lib/nav.tsx` (`buildNav`), shared by `Sidebar`, `MobileTabBar` and the palette's commands so the three cannot drift. Counts and the unread dot stay in `Sidebar` — `buildNav` returns structure only
- **Five CSS guard tests** exist because plan 1's hand-rewrite of `globals.css` broke each of these silently: `css-vars.test.ts` (no dangling `var(--…)`), `css-classes.test.ts` (every project class the markup references is defined), `svg-size.test.ts` (every `<svg>` declares its own size — an unsized one defaults to 300×150px), `legacy-classes.test.ts` (no Tailwind palette classes, radii, shadows, rings, hex literals, or a second breakpoint), and `class-fragments.test.ts` (no severed class tails such as `h-[11px]-[2px]` or a bare `group-`, left when a prefix was cut out)
- **Data colours** (task, habit, priority, chart segment) come from one muted palette in `src/lib/colors.ts`, validated to clear 3:1 on both themes. Records created before it still hold saturated Tailwind-500 hexes in the database; the axios response interceptor maps every `color` field onto the palette (`recolorLegacy`), so render sites never see the old values
- **One primary button style**: `.primary-btn` is a filled accent. Use the class (or `<Button>`) rather than hand-rolling `bg-accent text-bg` — its disabled state is a neutral cell, not a faded accent

**Pages:** Today, Tasks (`/tasks`, plus a per-board list view at `/list/:boardId`), Calendar (day/week/month views), Habits, Stats, Notifications, Admin, plus a `*` catch-all rendering `NotFoundPage` inside the shell. There is no Boards or Kanban screen: `/boards` and `/kanban` are legacy redirects in `App.tsx` to `/today` and `/tasks` respectively, with no page component behind them (no `BoardsPage.tsx` or `KanbanPage.tsx` exists in `src/pages/`). There is also no `/export` route and no `ExportPage.tsx` — the screen was deleted once the terminal redesign reached it, since nothing routed to it. The backend endpoints it used (`GET /api/export/tasks`, `GET /api/export/stats`) are untouched and still answer. (Notes were dropped from the schema before the Node port — there's no `Notes` model in `prisma/schema.prisma` — and removed from the UI. Budget has no frontend page — the API and all 9 Prisma budget models, two of which are join tables, remain unconsumed by the UI.)
