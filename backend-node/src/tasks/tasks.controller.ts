...
export class TasksController {
  ...
  reorder(@CurrentUser() user: User, @Body() body: KanbanReorderDto): Promise<{ ok: true }> {
  ...
  list(
    ...
    // Manual enum narrowing: Python's `Query(..., alias="status")` with a
    // Pydantic enum 422s on an unknown value. We mirror that via the DTO
    // layer on the body side, but query params don't get DTO'd — do it
    // here manually. Silent-ignore unknown values keeps parity with a
    // lenient GET.
    const statusFilter = (KANBAN_STATUS_VALUES as readonly string[]).includes(status ?? '')
      ? (status as KanbanStatusWire)
      : undefined;
