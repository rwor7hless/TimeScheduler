 ...
 * uploaded as soon as `Upload` collects them. Failures bubble up as thrown
 * errors (logged + surfaced to the admin endpoint); no Telegram pings.
 */
@Injectable()
export class S3BackupService {
  ...
  async run(): Promise<BackupResult> {
    ...
    try {
      ...
      if (size === 0) {
        throw new Error(
          'pg_dump produced 0 bytes — likely client/server version mismatch or auth failure. Check logs from PgDumpStreamer for stderr output.',
        );
      }
